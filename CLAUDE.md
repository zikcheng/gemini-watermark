...
4. **Float width**: C++ computes in `float` (32-bit); JS numbers are
   ...
   per-category budgets defined in PLAN.md (currently 2e-3 against the
   3-decimal manifest oracle).
...
- **Every algorithm module ships with equivalence tests in the same
  ...
  ±1 per channel; score assertions compare absolute differences against
  per-category `(absTol, relTol)` budgets defined in PLAN.md — never
  `toBeCloseTo` decimal-digit semantics. Threshold gates are extracted as
  pure functions and unit-tested at `<` / `==` / `>` directly.
