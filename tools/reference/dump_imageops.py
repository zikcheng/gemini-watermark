...
def synthetic_full_range() -> np.ndarray:
    """A 16x16 BGR image walking the full 0..255 range on every channel.

    Deterministic by construction, no randomness: 256 pixels, one per
    intensity, with the three channels offset against each other so no two
    channels ever carry the same value at the same pixel — which is what
    makes the grayscale coefficients separable in the output.
    """
    values = np.arange(256, dtype=np.uint8)
    b = values
    g = np.roll(values, 85)
    r = np.roll(values, 170)
    return np.stack([b, g, r], axis=1).reshape(16, 16, 3)
...
def gray_inputs() -> list:
    ...
    for case, file in REAL_INPUTS:
        entries.append((case, {"kind": "case_file", "path": f"{case}/{file}"},
                        real_bgr(case, file)))
    entries.append(("synthetic-16x16",
...
def match_template():
    ...
    record("varied-image-constant-template", varied.repeat(3, 0).repeat(3, 1).copy(),
